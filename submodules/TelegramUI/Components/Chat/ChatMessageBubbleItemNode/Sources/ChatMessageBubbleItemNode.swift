...
private func contentNodeMessagesAndClassesForItem(_ item: ChatMessageItem) -> ([(Message, AnyClass, ChatMessageEntryAttributes, BubbleItemAttributes)], Bool, Bool) {
    ...
    if !reactionsAreInline && !hideAllAdditionalInfo && !LuminaSettingsCache.shared.current().hideReactions, let reactionsAttribute = mergedMessageReactions(attributes: firstMessage.attributes, isTags: firstMessage.areReactionsTags(accountPeerId: item.context.account.peerId)), !reactionsAttribute.reactions.isEmpty {
...
public class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private static func beginLayout(
        ...
        if (needReactions || forceReactionsOutside) && !LuminaSettingsCache.shared.current().hideReactions {
