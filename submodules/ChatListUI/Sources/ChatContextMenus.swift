                ...
                |> map { [weak chatListController] isContact, notificationSettings, globalNotificationSettings, readCounters -> [ContextMenuItem] in
                    ...
                    if case .search(.recentApps) = source {
                        ...
                        if let group = peerGroup {
                            ...
                            if !isSavedMessages {
                                // LuminaGram: chat-lock / private folder. Locking removes this
                                // chat from the chat list and search until the secret code is
                                // typed into the search field (LuminaChatLock.swift) - display
                                // layer only, never touches read/typing/online state and never
                                // calls the server. Refuses to lock when no code is configured
                                // yet: locking then would hide the chat with no way to bring it
                                ...
                                items.append(.action(ContextMenuActionItem(text: isLocked ? LuminaL10n.tr("Unlock Chat") : LuminaL10n.tr("Lock Chat"), icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Lock"), color: theme.contextMenu.primaryColor) }, action: { _, f in
                                    if isLocked {
                                        ...
                                        f(.default)
                                    } else if LuminaChatLock.hasSecretCode() {
                                        LuminaChatLock.lock(peerId: peerId.toInt64(), accountManager: context.sharedContext.accountManager)
                                        f(.default)
                                    } else {
